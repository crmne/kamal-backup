...
| `backup` | Create one database snapshot per configured database and one file snapshot for configured paths. From an app checkout with `config/deploy.yml`, or with `-d`/`-c`, it runs on production infrastructure through Kamal. Remote execution requires the local gem and accessory versions to match. |
