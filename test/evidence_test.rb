  ...
  class ErrorRestic
    def latest_snapshot(**)
      raise KamalBackup::CommandError.new(
        'restic failed',
        command: KamalBackup::CommandSpec.new(argv: %w[restic snapshots]),
        status: 1,
  ...
  def test_evidence_includes_the_last_restore_drill
    Dir.mktmpdir do |dir|
      ...
      File.write(
        ...
        JSON.pretty_generate({ schema_version: 1, kind: 'drill_result', status: 'ok', scope: 'local',
  ...
  def test_evidence_includes_last_check
    Dir.mktmpdir do |dir|
      ...
      File.write(
        ...
        JSON.pretty_generate({ status: 'ok', started_at: '2026-04-23T10:00:00Z', finished_at: '2026-04-23T10:01:00Z',
                               output: 'no errors' })
      )
